...
impl Def {
    pub fn try_from(mut item_mod: ItemMod) -> Result<Self> {
        ...
        let items = &mut item_mod
            ...
            .as_mut()
            .ok_or_else(|| {
                let msg = "No content inside the PVQ program definition";
        ...
        for item in items.iter_mut() {
            let pvq_attr = helper::take_first_pvq_attr(item)?;

            if let Some(attr) = pvq_attr {
                if let Some(last_segment) = attr.path().segments.last() {
                    if last_segment.ident == "call_def" {
                        ...
                        return Err(Error::new(
                            item.span(),
                            "Invalid attribute, expected `#[pvq::call_def]` or `#[pvq::entrypoint]`",
                        ));
                    }
                }
            }

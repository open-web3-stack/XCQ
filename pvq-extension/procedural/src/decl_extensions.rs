...
fn add_super_trait(item_trait: &mut ItemTrait) -> Result<()> {
    ...
    // item_trait.supertraits.push(parse_quote!(#pvq_extension::ExtensionId));
    item_trait
        .supertraits
        .push(parse_quote!(#pvq_extension::ExtensionImplMetadata));
    Ok(())
}
...
fn impl_dispatchable(trait_ident: &Ident, methods: &[Method]) -> Result<ItemImpl> {
    ...
    Ok(parse_quote! {
        impl<Impl: #trait_ident> #pvq_extension::Dispatchable for Call<Impl> {
...
fn impl_extension_id(trait_ident: &Ident, trait_items: &[TraitItem]) -> Result<TokenStream2> {
    ...
    Ok(quote! {
        ...
        impl<Impl: #trait_ident> #pvq_extension::ExtensionId for Call<Impl> {
        ...
        pub const EXTENSION_ID: #pvq_extension::ExtensionIdTy = #extension_id;
    })
}
...
fn impl_call_metadata(trait_ident: &Ident) -> Result<ItemImpl> {
    let pvq_extension = generate_crate_access("pvq-extension")?;
    Ok(parse_quote! {
        impl<Impl: #trait_ident> #pvq_extension::CallMetadata for Call<Impl> {
            fn call_metadata() -> #pvq_extension::metadata::ExtensionMetadata {
                Impl::extension_metadata(<Self as #pvq_extension::ExtensionId>::EXTENSION_ID)
            }
        }
    })
}
